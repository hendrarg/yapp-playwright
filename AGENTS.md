> **Obsidian:** [[projects/yapp/yapp|yapp]] · [[projects/yapp/knowledge/index|Knowledge index]]
...
**Knowledge lives in Obsidian.** The canonical files are in the Obsidian vault at `D:/Knowledge/projects/yapp/knowledge/`. In this repo, `.agents/knowledge/` is a **directory junction** to that vault folder — agents still read and write the same path; edits in Obsidian or Cursor update one file. New clones need the junction: run `scripts/setup-obsidian-junction.ps1` (requires the vault at `D:/Knowledge`). Optional env override: `YAPP_OBSIDIAN_VAULT` if your vault path differs.
