> **Obsidian:** [[projects/yapp/yapp|yapp]] · [[projects/yapp/knowledge/index|Domain knowledge index]]
...
When the task touches a product area, read the matching note in `.agents/knowledge/` — start from `.agents/knowledge/index.md`. Those notes record how the app actually behaves on dev (defaults, validation boundaries, fixtures, UI traps) and exist because an earlier session assumed the obvious behavior and was wrong. They describe observed behavior, so they rank below this guide and `.agents/rules/`, and below the sheets on anything the sheets state.

**Reading is half the rule — write back too.** Whenever the browser or the API contradicts an assumption you started with, or you establish a product fact that is not already recorded, add it to the matching feature file in the same session. Product behavior goes to `.agents/knowledge/`; locator and Playwright technique goes to `.agents/rules/mcp-playwright.md`.

**Domain knowledge lives in Obsidian.** The canonical files are in the Obsidian vault at `D:/Knowledge/projects/yapp/knowledge/`. In this repo, `.agents/knowledge/` is a **directory junction** to that vault folder — agents still read and write the same path; edits in Obsidian or Cursor update one file. New clones need the junction: run `scripts/setup-obsidian-junction.ps1` (requires the vault at `D:/Knowledge`). Optional env override: `YAPP_DOMAIN_KNOWLEDGE_VAULT` if your vault path differs.
...
2. Read the matching `.agents/knowledge/` note for the product area, if one exists.
...
.agents/                runtime.md, rules/, skills/, commands/, knowledge/ — the only place to edit
.agents/knowledge/   junction → Obsidian vault; verified Yapp product behavior (start at index.md)
CLAUDE.md               one line (@AGENTS.md) so Claude Code auto-loads the guide
opencode.json           points OpenCode at .agents/rules/* and .agents/knowledge/
