...
const accounts = [
  { label: 'token1', envVar: 'YAPP_TEST_ACCESS_TOKEN', expected: 'jendraljohn92', name: 'Hendra' },
...
function status(token, expected) {
  ...
  if (!username) return 'unreadable JWT';
  ...
  if (token) {
    try {
      const claims = JSON.parse(
        Buffer.from(token.replace(/^"|"$/g, '').trim().split('.')[1].replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8'),
      );
      if (claims.exp) {
...
console.log('OTP login inbox: %s.sdet@inbox.testmail.app → saves to YAPP_TEST_ACCESS_TOKEN_2 (Sundanese only)', process.env.TESTMAIL_NAMESPACE ?? '?');
